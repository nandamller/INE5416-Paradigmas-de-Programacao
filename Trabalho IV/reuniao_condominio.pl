...
imediatamenteEsquerda(X, Y, Lista) :- nextto(X, Y, Lista).
imediatamenteDireita(X, Y, Lista) :- nextto(Y, X, Lista).
...
solucao(ListaSolucao) :- 
    ...
    imediatamenteEsquerda(morador(_, _, _, _, _, joalheiro), morador(_, _, _, _, 45, _), ListaSolucao),

    % César está exatamente à esquerda de quem está reclamando de Barulho.
    imediatamenteEsquerda(morador(_, cesar, _, _, _, _), morador(_, _, _, barulho, _, _), ListaSolucao),
    ...
    aDireita(morador(verde, _, _, _, _, _), morador(vermelha, _, _, _, _, _), ListaSolucao),
    aEsquerda(morador(verde, _, _, _, _, _), morador(_, _, _, _, 35, _), ListaSolucao),
    ...
    imediatamenteDireita(morador(branca, _, _, _, _, _), morador(_, _, _, manutencao, _, _), ListaSolucao),
    ...
    imediatamenteDireita(morador(_, _, _, _, _, arquiteto), morador(_, _, _, garagem, _, _), ListaSolucao),
    ...
    imediatamenteDireita(morador(_, juan, _, _, _, _), morador(_, _, _, _, _, joalheiro), ListaSolucao),

    % % O morador de Azul está exatamente à esquerda de quem está reclamando da Manutenção.
    imediatamenteEsquerda(morador(azul, _, _, _, _, _), morador(_, _, _, manutencao, _, _), ListaSolucao),

    % % O condômino do apartamento 32 está exatamente à esquerda do Comerciante.
    imediatamenteEsquerda(morador(_, _, 32, _, _, _), morador(_, _, _, _, _, comerciante), ListaSolucao),
    ...
    aDireita(morador(_, vicente, _, _, _, _), morador(_, rodolfo, _, _, _, _), ListaSolucao),
    aEsquerda(morador(_, vicente, _, _, _, _), morador(_, juan, _, _, _, _), ListaSolucao),

    % % O condômino mais velho está exatamente à esquerda do Farmacêutico.
    imediatamenteEsquerda(morador(_, _, _, _, 50, _), morador(_, _, _, _, _, farmaceutico), ListaSolucao),
